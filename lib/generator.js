...
exports.routeToSwaggerPaths = function routeToSwaggerPaths(route, options) {
  ...
  if (route.validate) {
    ...
    const defaultDescriptions = options.defaultDescriptions || {};
    ...
    if (output) {
      Object.keys(output).forEach(x => {
        ...
        respCodes.forEach(code => {
          ...
          code = code.trim();
          let defaultDescription = defaultDescriptions[x] || defaultDescriptions.default;
          routeDesc.responses[code] = outputToSwagger(output[x], defaultDescription);
        });
      });
    }
  }
...
function outputToSwagger (respJson, defaultDescription) {
  let obj = {};
  if (respJson && respJson.body) {
    obj.schema = j2s(respJson.body).swagger;
    obj.description = respJson.schema || obj.schema.description || defaultDescription || 'Success';
  }
  return obj;
}
